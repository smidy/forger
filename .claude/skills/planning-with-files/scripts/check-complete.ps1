...
if (-not $PlanFile) {
    ...
    $finder = Join-Path $scriptDir "find-active-plan.ps1"
    if (Test-Path $finder) {
        try {
            $dir = (& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $finder 2>$null | Out-String).Trim()
            if ($dir) { $PlanFile = Join-Path $dir "task_plan.md" }
        } catch { }
    }
}

if (-not $PlanFile -or -not (Test-Path $PlanFile)) {
    Write-Host '[planning-with-files] No active plan found -- no active planning session.'
    exit 0
}
