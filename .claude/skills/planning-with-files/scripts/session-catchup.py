...
def main():
    ...
    # Check if any plan directory exists with the triad (indicates active task).
    # Plans live at docs/plans/<YYYYMMDD>-<slug>/ — scan for non-archive subdirs
    # that contain at least one of the triad files.
    plans_root = Path(project_path, 'docs', 'plans')
    has_planning_files = False
    if plans_root.is_dir():
        for plan_dir in plans_root.iterdir():
            if not plan_dir.is_dir() or plan_dir.name.startswith('_'):
                continue
            if any((plan_dir / f).exists() for f in PLANNING_FILES):
                has_planning_files = True
                break
    ...
    print("2. Resolve active plan: scripts/find-active-plan.sh (or .ps1)")
    print("3. Read: <plan-dir>/{task_plan.md, progress.md, findings.md}")
