...
if [ -z "$PLAN_FILE" ]; then
    ...
    if [ -x "$SCRIPT_DIR/find-active-plan.sh" ] || [ -f "$SCRIPT_DIR/find-active-plan.sh" ]; then
        DIR=$(sh "$SCRIPT_DIR/find-active-plan.sh" 2>/dev/null) || DIR=""
        if [ -n "$DIR" ]; then
            PLAN_FILE="$DIR/task_plan.md"
        fi
    fi
fi

if [ -z "$PLAN_FILE" ] || [ ! -f "$PLAN_FILE" ]; then
    echo "[planning-with-files] No active plan found — no active planning session."
    exit 0
fi
